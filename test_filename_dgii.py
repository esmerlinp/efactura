...
from app.services.db_service import get_company
import time

def test_filename(filename, encf="E310000000001"):
    company = get_company("eG1D24i5oYvT6i9M4BIf")
    company_profile = company.get("dgiiProfile", {})
    token = DgiiDirectService.get_dgii_token(company_profile, sandbox=True)
    
    xml_data = b"<ECF></ECF>" # It will probably fail XML validation, but let's test a valid XML
