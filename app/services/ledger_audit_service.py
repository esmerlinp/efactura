...
class LedgerAuditService:
    ...
    @staticmethod
    def _audit_path(owner_uid: str, entry_id: str, audit_id: str = None):
    ...
    @staticmethod
    def log_entry_creation(entry: dict, owner_uid: str, performed_by: str = ""):
        try:
            ...
            audit_id = str(uuid4())
            doc_ref = db.document(LedgerAuditService._audit_path(owner_uid, entry_id, audit_id))
            doc_ref.set({
                "action": "CREATE",
                "entry_id": entry_id,
    ...
    @staticmethod
    def log_entry_update(old_entry: dict, new_entry: dict, owner_uid: str, performed_by: str = ""):
        try:
            db = LedgerAuditService._get_db()
    ...
    @staticmethod
    def log_entry_void(entry: dict, owner_uid: str, performed_by: str = "", reason: str = ""):
        try:
            db = LedgerAuditService._get_db()
            entry_id = entry.get('id', '')
            if not entry_id:
    ...
    @staticmethod
    def get_entry_audit_log(owner_uid: str, entry_id: str) -> list:
        try:
            db = LedgerAuditService._get_db()
            docs = db.collection(LedgerAuditService._audit_path(owner_uid, entry_id) + "/changes") \
