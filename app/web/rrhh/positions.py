...
def position_list():
    ...
    items = hr.get_catalog(owner_uid, "positions", sandbox=sandbox)
...
def position_save():
    ...
    if name:
        hr.save_catalog_item(owner_uid, "positions", {
...
def position_delete(item_id):
    if _login_required():
        return redirect(url_for("web_auth.login"))
    owner_uid, sandbox = _get_owner_uid_and_sandbox()
    hr.delete_catalog_item(owner_uid, "positions", item_id, sandbox=sandbox)
