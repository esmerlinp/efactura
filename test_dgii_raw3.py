...
docs = db.collection('companies').limit(10).stream()
company = None
for d in docs:
    c = d.to_dict()
    if 'dgiiProfile' in c and c['dgiiProfile'].get('certificateContent'):
        company = c
        break

if not company:
    print("No company found with dgiiProfile!")
    exit(1)
...
test_file("133753652E310000000004")
