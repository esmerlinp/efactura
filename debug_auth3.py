...
for user_doc in users:
    ...
    for comp_doc in companies:
        ...
        if not req_ref.exists:
            req_ref = db_firestore.collection('companies').document(owner_uid).collection('company').document(company_id).collection('hr_authorization_requests').document(req_id).get()
            
        if req_ref.exists:
            print("FOUND REQUEST:", req_ref.reference.path)
            req_data = req_ref.to_dict()
            print("STATUS:", req_data.get("status"))
            print("isFallback:", req_data.get("isFallback"))
            print("ruleId:", req_data.get("ruleId"))
            print("createdByUid:", req_data.get("createdByUid"))
            print("createdByEmail:", req_data.get("createdByEmail"))
            print("STEPS:")
